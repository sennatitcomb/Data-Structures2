...
int queue_from_stacks_isempty(struct queue_from_stacks* qfs) {
  if(stack_isempty(qfs->s2) == 1) { //tests second stack
    if(stack_isempty(qfs->s1) == 1) { //tests first stack
      return 1;
    } else {
      return 0;
    }
...
void* queue_from_stacks_front(struct queue_from_stacks* qfs) {
  if (stack_isempty(qfs->s2) == 0 && stack_isempty(qfs->s1) == 0) {
    return stack_top(qfs->s1);
  } else if (stack_isempty(qfs->s2) == 0 && stack_isempty(qfs->s1) == 1) {
...
void* queue_from_stacks_dequeue(struct queue_from_stacks* qfs) {
  if (stack_isempty(qfs->s2) == 0) { //if second stack is not empty
